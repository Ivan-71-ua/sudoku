...
public class ShuduCanvers extends JPanel implements MouseListener {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				...
				if (passRole(ShuduMainFrame.pass)) {
					...
					cells[i][j].setForeground(new Color(255, 253, 208));
				} else {
	...
	private void checkFinish() {
		...
		if (ShuduMainFrame.pass > 2) {
			int o = JOptionPane
					.showConfirmDialog(this, "You have completed the level. Want to start over?", "", 0);
			if (o == 1) {
				System.exit(0);
			} else {
				ShuduMainFrame.pass = 1;
			...
			JOptionPane.showMessageDialog(this, "Congratulations on completing this level! Spent time: "
	...
	private Color getColor(int i) {
		return new Color(0xFFA32B4C, true);
	}
	/*private Color getColor(int i) {
		Color color = Color.pink;
		switch (i) {
			case 1:
				color = new Color(163, 43, 76);
				break;
			case 2:
				color = new Color(163, 43, 76);
				break;
